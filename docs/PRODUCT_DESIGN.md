...
Pre-Anything 是一个面向 macOS 的开发者文件预览工具。它通过 Quick Look，让用户在 Finder 中无需打开编辑器，就能快速理解 Markdown 文档和结构化配置文件。
...
Pre-Anything 的定位不是通用代码查看器，也不是以格式数量取胜的 Quick Look 插件集合。
...
长期差异化来自“理解文件”，而不是“给文件上色”。格式支持达到基本完整后，产品应优先发展 OpenAPI、Kubernetes、Docker Compose、项目清单等语义预览，而不是盲目增加数百种源码后缀。
...
- 阅读 AI、自动化工具或同事生成的 Markdown、JSON、YAML 文件的人。
...
新增格式必须带来明显优于系统纯文本预览的体验。仅仅增加语法颜色，不足以成为长期支持某个格式的理由。
...
2. 列出 Markdown、JSON、YAML 三个独立扩展。
3. 引导用户进入 macOS 系统设置启用或停用各格式扩展。
4. 分别设置 Markdown、JSON、YAML 是否使用透明背景，并可一次应用到全部格式。
...
- Markdown、JSON 和 YAML 的主画布都不主动绘制底色，透出 Quick Look 宿主背景；代码块、行内代码、表格和诊断信息仍可保留自身的内容背景。
...
- Markdown、JSON、YAML 扩展及其用途说明。
- Markdown、JSON、YAML 独立透明背景开关与全选操作。
- 打开 macOS 扩展管理入口的操作。
- 必要的版本与开源许可信息。
...
- Markdown、JSON、YAML 分别使用独立扩展，使用户可以在 macOS 中真实启停单个格式，并允许其他扩展接管。
...
- 仅使用 `group.com.lixinlv.PreAnything` App Group 同步三种格式的透明背景偏好，不承载文件内容或其他共享状态。
...
- Markdown、JSON、YAML。
- Markdown 原生阅读、JSON 格式化高亮、YAML 原文高亮。
- 深浅色、错误提示、编码处理和大文件降级。
- 三个格式扩展可在 macOS 中独立启停。
